...
import useSWR from "swr"; // Keep useSWR for main venta and detalleVenta
...
export default function VentaDetallePage() {
  ...
  } = useSWR<VentaGet>(id ? `/api/venta/venta/${id}` : null, apiFetcher);
  ...
  const clienteMap = useMemo(
    () => new Map(clientes?.map((c) => [c.id, `${c.nombre} ${c.apellido}`])),
    [clientes]
  );
  const vendedorMap = useMemo(
    () => new Map(vendedores?.map((v) => [v.id, `${v.nombre} ${v.apellido}`])),
    [vendedores]
  );
  const prodVarianteMap = useMemo(
    () => new Map(prodVariantes?.map((pv) => [pv.id, pv])),
    [prodVariantes]
  );
  ...
  if (isLoading)
    return (
      <div className="text-center py-10">Cargando detalles de la venta...</div>
    );
  if (error)
    return (
  ...
  if (!venta)
    return <div className="text-center py-10">Venta no encontrada.</div>;
    ...
    <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-6xl mx-auto">
      <div className="px-4 py-5 sm:px-6 flex justify-between items-center border-b border-gray-200">
        <div>
          ...
          <h3 className="text-lg leading-6 font-medium text-gray-900">
            Detalles de la Venta #{venta.numeroVenta}
          </h3>
      ...
      <div className="border-t border-gray-200">
        <dl>
          <DetailItem
            label="Cliente"
            value={clienteMap.get(venta.clienteId) || venta.clienteId}
          />
          <DetailItem
            label="Vendedor"
            value={vendedorMap.get(venta.vendedorId) || venta.vendedorId}
          />
          <DetailItem
            label="Fecha de Venta"
            value={formatDate(venta.fechaVenta)}
          />
