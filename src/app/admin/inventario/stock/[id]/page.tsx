  ...
  if (isLoading)
    return (
      <div className="text-center py-10">
        Cargando detalles de la variante...
      </div>
    );
  if (error)
    return (
      <div className="text-center py-10 text-red-500">
        Error al cargar la variante.
      </div>
    );
  if (!variante)
    return <div className="text-center py-10">Variante no encontrada.</div>;
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            ...
            <span className="text-lg font-normal text-gray-600 ml-2">
              ({variante.color.nombre}, {variante.talla.talla})
            </span>
          </h1>
        </div>
        <Link
          href={`/admin/inventario/stock/${id}/editar`}
