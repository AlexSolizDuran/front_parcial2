...
interface EtiquetaFormModalProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  etiquetaParaEditar?: EtiquetaGet | null;
}

export default function EtiquetaFormModal({
  isOpen,
  onClose,
  onSuccess,
  etiquetaParaEditar,
}: EtiquetaFormModalProps) {
  const [data, setData] = useState<EtiqueteSet>({
    nombre: "",
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && etiquetaParaEditar) {
        setData({ nombre: etiquetaParaEditar.nombre });
      } else {
        setData({ nombre: "" });
      }
      setError(null);
  ...
  }, [isOpen, isEditMode, etiquetaParaEditar]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditMode && etiquetaParaEditar) {
        await apiFetcher(`/api/producto/etiqueta/${etiquetaParaEditar.id}`, {
          method: "PUT",
          body: JSON.stringify(data),
        });
      } else {
        await apiFetcher("/api/producto/etiqueta", {
          method: "POST",
          body: JSON.stringify(data),
        });
      }
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex justify-between items-center p-4 border-b">
            <h3 className="text-lg font-bold">
              {isEditMode ? "Editar Etiqueta" : "Nueva Etiqueta"}
            </h3>
            <button
              type="button"
              onClick={onClose}
