...
export default function MarcaFormModal({
  ...
  const [data, setData] = useState<MarcaSet>({
    nombre: "",
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && marcaParaEditar) {
        setData({ nombre: marcaParaEditar.nombre });
      } else {
        setData({ nombre: "" });
      }
      setError(null);
  ...
  }, [isOpen, isEditMode, marcaParaEditar]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditMode && marcaParaEditar) {
        await apiFetcher(`/api/producto/marca/${marcaParaEditar.id}`, {
          method: "PUT",
          body: JSON.stringify(data),
        });
      } else {
        await apiFetcher("/api/producto/marca", {
          method: "POST",
          body: JSON.stringify(data),
        });
      }
