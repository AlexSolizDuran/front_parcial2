...
export default function Sidebar() {
    ...
    const menuItems: MenuItem[] = [
        { name: "Panel Principal", href: "/admin", icon: <LayoutDashboardIcon className="h-5 w-5" /> },
        {
            name: "Ventas & Caja",
            ...
            >
                ...
                <nav className="flex-1 p-3 space-y-2 font-medium">
                    {menuItems.map((item, i) => {
                        ...
                        const baseClasses = "w-full flex items-center p-3 rounded-lg transition-all duration-150 group";
                        // Clase de estado normal/hover
                        const defaultClasses = "text-gray-300 hover:bg-slate-800 hover:text-white";
                        // Clase de estado activo (El acento dorado)
                        const activeClasses = "bg-amber-500 text-slate-900 shadow-md font-semibold";
                        ...
                        return (
                            <div key={i}>
                                {!hasChildren ? (
                                    ...
                                    >
                                        <span className={`mr-3 ${itemIsActive ? 'text-slate-900' : 'text-amber-500 group-hover:text-white'}`}>{item.icon}</span>
                                        <span>{item.name}</span>
                                    </Link>
                                ) : (
